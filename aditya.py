...
from ibran import overlay_with_autofit, overlay_sunglasses, overlay_thug_life  # Importing from ibran.py
from avt import apply_bw_filter  # Importing from avt.py
...
# Sidebar options
mode_option = st.sidebar.selectbox("Choose mode:", ["Processed Overlay", "Original Overlay"])
...
if mode_option == "Processed Overlay":
    st.write("This mode applies changes to the avatar image.")
else:
    st.write("This mode applies changes to the original image.")
...
if option == "Upload Image":
    ...
    if uploaded_file is not None:
        ...
        if results.detections:
            ...
            # Apply filters based on the selected mode and option
            if mode_option == "Processed Overlay":
                if filter_option == "Robot":
                    ...
                    img_with_overlay = apply_bw_filter(img.copy())  # Apply B/W filter to the original image
            else:  # Original Overlay
                if filter_option == "Robot":
                    img_with_overlay = overlay_with_autofit(img.copy(), robot_face_image, face_coordinates)
                elif filter_option == "Sunglasses":
                    img_with_overlay = overlay_with_autofit(img.copy(), sunglasses_image, face_coordinates, is_lens_filter=True)
                elif filter_option == "Thug Life":
                    img_with_overlay = overlay_with_autofit(img.copy(), thug_life_image, face_coordinates)
                elif filter_option == "Spiderman":
                    img_with_overlay = overlay_with_autofit(img.copy(), spiderman_image, face_coordinates)
                elif filter_option == "Alien":
                    img_with_overlay = overlay_with_autofit(img.copy(), alien_image, face_coordinates)
